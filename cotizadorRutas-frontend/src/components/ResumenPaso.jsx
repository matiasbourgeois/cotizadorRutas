...
// Componente interno para un ítem de desglose de costos
         ...
         <Stack gap="xs">
            {recursoHumano ? (
                <Stack pl={34} gap="xs">
                    ...
                    {detalleRecurso ? (
                        <>
                            <CostoDesglosadoItem label="Salario y Cargas Sociales" valor={detalleRecurso.detalle.sueldoProporcional + detalleRecurso.detalle.adicionalActividad + detalleRecurso.detalle.cargasSociales} />
                            <CostoDesglosadoItem label="Viáticos y Adicionales" valor={detalleRecurso.detalle.adicionalKm + detalleRecurso.detalle.viaticoKm + detalleRecurso.detalle.adicionalFijo} />
                            <Divider style={{ width: '50%', alignSelf: 'flex-end' }} />
                            <Group justify="flex-end"><Text fz="sm" fw={700}>Subtotal RRHH:</Text><Text fz="sm" fw={700}>${(detalleRecurso.totalFinal || 0).toLocaleString('es-AR')}</Text></Group>
                        </>
                    ) : <Center><Loader size="xs" type="dots" /></Center>}
                </Stack>
            ) : <Text size="sm" c="dimmed" pl={34}>Pendiente de selección...</Text>}
    ...
    return (
        <Paper withBorder p="lg" radius="md" shadow="sm">
            <Stack gap="lg">
                ...
                {/* ✅ Lógica condicional: Si es el paso final, usamos un Accordion para el desglose */}
                {esPasoFinal ? (
                    ...
                    // Si no es el paso final, mostramos la vista normal que ya te gustaba
                    <>
                        <InfoItem icon={<MapPin size={16} />} label="Distancia por viaje">{puntosEntrega.distanciaKm?.toFixed(2) || 'N/A'} km</InfoItem>
                        <InfoItem icon={<Calendar size={16} />} label="Frecuencia">{frecuenciaTexto}</InfoItem>
                        <Divider />
                ...
                {/* ✅ La sección de Propuesta Final solo se muestra en el último paso */}
