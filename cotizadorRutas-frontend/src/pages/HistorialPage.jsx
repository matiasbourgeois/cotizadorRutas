...
import { Title, Table, ScrollArea, Group, Button, Text, Center, Loader, Paper, ActionIcon, Tooltip, Badge, Stack } from '@mantine/core';
import { Link, useNavigate } from 'react-router-dom';
import { Download, Trash2, FileX, Plus, Route } from 'lucide-react';
...
const HistorialPage = () => {
  ...
  const handleDescargarPdf = async (id, e) => {
    ...
    try {
      const res = await clienteAxios.get(`/presupuestos/${id}/pdf`, { responseType: 'blob' });
        ...
        <Table.Td>
          <Group gap="xs" justify="flex-end">
            <Tooltip label="Descargar PDF">
              <ActionIcon variant="light" color="blue" onClick={(e) => handleDescargarPdf(presupuesto._id, e)}>
                <Download size={16} />
              </ActionIcon>
            </Tooltip>
            <Tooltip label="Eliminar">
              <ActionIcon variant="light" color="red" onClick={() => handleEliminar(presupuesto._id)}>
                <Trash2 size={16} />
              </ActionIcon>
            </Tooltip>
          </Group>
