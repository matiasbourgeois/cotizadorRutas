...
const HistorialPage = () => {
  ...
  const rows = presupuestos.map((presupuesto) => {
    ...
    const destino = presupuesto.puntosEntrega?.length > 1 ? presupuesto.puntosEntrega[presupuesto.puntosEntrega.length - 1]?.nombre?.split('–')[0]?.trim() || 'Sin destino' : 'Vuelta en el lugar';
    ...
    return (
      <Table.Tr key={presupuesto._id}>
        ...
        <Table.Td><Text fw={500} ta="right">${(presupuesto.resumenCostos?.totalFinal ?? 0).toLocaleString('es-AR')}</Text></Table.Td>
        <Table.Td>
